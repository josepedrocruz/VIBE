    ...
    if not args.no_render:
        ...
        for frame_idx in tqdm(range(len(image_file_names))):
            ...
            for person_id, person_data in frame_results[frame_idx].items():
                ...
                # Render the person onto the white image
                white_img = renderer.render(white_img, frame_verts, cam=frame_cam, color=mesh_color[person_id])
            ...
            cv2.imwrite(os.path.join(output_img_folder, f'{frame_idx:05d}.png'), white_img)
            
            #cv2.imwrite(os.path.join(output_img_folder, f'{frame_idx:06d}.png'), img)

            if args.display:
                cv2.imshow('Video', white_img)
